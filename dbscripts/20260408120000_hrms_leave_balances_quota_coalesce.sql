-- Fix mobile (and RPC) showing ∞ for PL/SL: use COALESCE(policy.annual_quota, leave_type.annual_quota).
-- Also add p_as_of so balances can use the employee's local calendar day (optional from client).
--
-- hrms_leave_year_start is defined here too so this file works even if 20260408110000 was not applied.
...
begin
  ...
  loop
    ...
    if v_as_of < v_eligible_start then
      ...
      v_entitled := m * rate;
      if v_eff_quota is not null then
        v_entitled := least(v_entitled, v_eff_quota);
      end if;
      ...
      -- annual (default)
      if v_eff_quota is null then
        v_entitled := 0;
      else
        v_entitled := greatest(0, v_eff_quota);
      end if;
    end if;
